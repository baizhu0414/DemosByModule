...
public class StepService extends Service implements SensorEventListener {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.O) {
            ...
            builder = new Notification.Builder(getApplicationContext(), Constant.CHANNEL_ID);
         ...
         * 总之有需要的可以和我交流
         */
        nfIntent = new Intent(this, CounterActivity.class);
        builder.setContentIntent(PendingIntent.getActivity(this, 0, nfIntent, 0)) // 设置PendingIntent
                .setLargeIcon(BitmapFactory.decodeResource(this.getResources(), R.mipmap.ic_launcher)) // 设置下拉列表中的图标(大图标)
                .setContentTitle("今日步数"+ mCurrentStep +"步") // 设置下拉列表里的标题
                .setSmallIcon(R.mipmap.ic_launcher) // 设置状态栏内的小图标
                .setContentText("加油，要记得勤加运动"); // 设置上下文内容
        // 获取构建好的Notification
        Notification stepNotification = builder.build();
        notificationManager.notify(Constant.NOTIFY_ID,stepNotification);
        // 参数一：唯一的通知标识；参数二：通知消息。
        startForeground(Constant.NOTIFY_ID, stepNotification);// 开始前台服务
        return START_STICKY;
    }
    /**
     * 自定义handler
